         ...
         Client.cmd.on('cecan', async(data) => {
          ...
          sec.push({
            "rows": [
          	...
          	let po = client.prepareMessageFromContent(data.from, {
				  "listMessage":{
                  "title": `MENU J-BOT`,
                  "description": `Hai Kak @${data.sender.split('@')[0]}, Semoga harimu menyenangkan\n\n${configs.botname} By Akmalzz`,
          ...
          sec.push({
            ...
            data.reply(`Owner number : @${628885960825}`) 
              ...
              Client.cmd.on('tesq', async(data) => {
              	try {
const buttons = [
  {buttonId: `${data.prefix}makasih`, buttonText: {displayText: 'Satisfied'}, type: 1},
  {buttonId: `${data.prefix}makasih`, buttonText: {displayText: 'Less satisfied'}, type: 1}
]
const buttonMessage = {
    contentText: menu(data.prefix, client, data),
    footerText: ' Rate The Bot Below Here :',
    buttons: buttons,
    headerType: 4, // change for file type
    }
client.sendMessage(data.from, buttonMessage, MessageType.buttonsMessage, {quoted: data.message})
} catch(e) {
                     data.reply('' + e) 
                   }
              })
              Client.cmd.on('makasih', async(data) => {
              	try {
              	data.reply('Terimakasih atas masukan anda :)')
              } catch(e) {
                     data.reply('' + e) 
                   }
              })
