    ...
    try {
        ...
        Client.cmd.on('ytmp4', async (data) => {
            try {
                ...
                res = await axios.get(`https://api-ezt.herokuapp.com/api/yutub/video?url=${data.body}&apikey=${configs.eztkey}`)
                if(res.data.status == false) data.reply(res.data.message)
                ytm = res.data.result
                teks = `*Data berhasil didapatkan!*\n\n*Judul* : ${ytm.title}\n*Ukuran* : ${ytm.filesize}\n*Kualitas* : ${ytm.resolution}\n*Ext* : ${ytm.ext}\n\n_Silahkan tunggu file media sedang dikirim mungkin butuh beberapa menit_`
                if(Number(ytm.filesize.split(' MB')[0]) >= 50.00) return Client.sendFileFromUrl(data.from, `${ytm.thumb}`, 'thumb.jpg', `*Data Berhasil Didapatkan!*\n\n*Judul* : ${ytm.title}\n*Ukuran* : ${ytm.filesize}\n*Kualitas* : ${ytm.resolution}\n*Ext* : mp4\n*Link* : ${ytm.result}\n\n_Jika durasi melebihi batas akan dikirim berbentuk link._`, data.message)
                Client.sendFileFromUrl(data.from, `${ytm.thumb}`, 'thumb.jpg', teks, data.message)
                Client.sendFileFromUrl(data.from, `${ytm.result}`, `${ytm.title} - Download.mp4`, `Video telah terkirim @${data.sender.split('@')[0]}`, data.message)
            } catch {
        ...
        Client.cmd.on('ytmp3', async (data) => {
            try {
                ...
                data.reply(mess.wait)
                res = await axios.get(`https://api-ezt.herokuapp.com/api/yutub/audio?url=${data.body}&apikey=${configs.eztkey}`)
                if(res.data.status == false) data.reply(res.data.message)
                ytm = res.data.result
                teks = `*Data berhasil didapatkan!*\n\n*Judul* : ${ytm.title}\n*Ukuran* : ${ytm.filesize}\n*Ext* : ${ytm.ext}\n\n_Silahkan tunggu file media sedang dikirim mungkin butuh beberapa menit_`
                if(Number(ytm.filesize.split(' MB')[0]) >= 50.00) return Client.sendFileFromUrl(data.from, `${ytm.thumb}`, 'thumb.jpg', `*Data Berhasil Didapatkan!*\n\n*Judul* : ${ytm.title}\n*Ukuran* : ${ytm.filesize}\n*Ext* : mp3\n*Link* : ${ytm.result}\n\n_Jika durasi melebihi batas akan dikirim berbentuk link._`, data.message)
                Client.sendFileFromUrl(data.from, `${ytm.thumb}`, 'thumb.jpg', teks, data.message)
                Client.sendFileFromUrl(data.from, `${ytm.result}`, `${ytm.title} - Download.mp3`, ``, data.message)
            } catch {
                data.reply('Ups maaf server sedang error')
            }
        }) 
       ...
       try {
        ...
        Client.cmd.on('asupan', async (data) => {
            ...
            if(data.args[0].toLowerCase() == 'ukhty') {
              Client.sendFileFromUrl(data.from, 'https://dapuhy-api.herokuapp.com/api/asupan/asupanukhty?apikey=BryanRfly', 'ukhty.mp4', `Nih Asupannya`, data.message)
            } else if(data.args[0].toLowerCase() == 'santuy') {
              Client.sendFileFromUrl(data.from, 'https://dapuhy-api.herokuapp.com/api/asupan/asupan?apikey=BryanRfly', 'santuy.mp4', `Nih Kak Asupannya`, data.message)
            } else if(data.args[0].toLowerCase() == '+62') {
              Client.sendFileFromUrl(data.from, 'https://dapuhy-api.herokuapp.com/api/asupan/asupan?apikey=BryanRfly', '+62.mp4',`Nih Kak Asupannya`, data.message)
            } else if(data.args[0].toLowerCase() == 'bocil')  {
              Client.sendFileFromUrl(data.from, 'https://dapuhy-api.herokuapp.com/api/asupan/asupanbocil?apikey=BryanRfly', 'bocil.mp4', `Nih Kak Asupannya`, data.message)
            } else if(data.args[0].toLowerCase() == 'rikagusriani') {
              Client.sendFileFromUrl(data.from, 'https://dapuhy-api.herokuapp.com/api/asupan/asupanrikagusriani?apikey=BryanRfly', 'rika.mp4', `Nih Kak Asupannya`, data.message)
        ...
        Client.cmd.on('randomimage', async (data) => {
            ...
            if(data.args[0].toLowerCase() == 'cecan') {
              ...
              Client.sendFileFromUrl(data.from, 'https://api.lolhuman.xyz/api/random/wallnime?apikey=KopiLuwak', 'wallnime.jpeg', `Nih Ramdom Imagenya`, data.message)
              		...
              		let po = client.prepareMessageFromContent(data.from, {
					"listMessage":{
                  "title": `Menu Random Image Permintaan Dari ${data.pushname}`,
                  "description": "Pilih Di Bawah Ini!",
                  "buttonText": "Klik Disini",
                  "listType": "SINGLE_SELECT",
                  "sections": [
...
`
        ...
        Client.cmd.on('sha512enc', async(data) => {
         ...
         Client.cmd.on('cecan', async(data) => {
          ...
          sec.push({
            "rows": [
          	...
          	let po = client.prepareMessageFromContent(data.from, {
				  "listMessage":{
                  "title": `MENU J-BOT`,
                  "description": `Hai Kak *${data.pushname}*, Semoga harimu menyenangkan\n\n${configs.botname} By Akmalzz`,
          ...
          sec.push({
            ...
            data.reply(`Owner number : @${628885960825}`) 
        }) 
    ...
    try {
    if (data.isGroup) {
      data.reply('Untuk Menghindari Anti link Pada Bot Lain Maka Link Grup J-BOT Akan Di Kirim Melalui Chat Pribadi!')
      client.sendMessage(data.sender, `*Link Grup J-Bot*\n\nhttps://chat.whatsapp.com/C9azc9XSbbeDZ7nNAmkCOl\nSilahkan Masuk Ya Kak ${data.pushname}`, MessageType.text, {quoted: data.message}) 
    } else client.sendMessage(data.from, `*Link Grup J-Bot*\n\nhttps://chat.whatsapp.com/C9azc9XSbbeDZ7nNAmkCOl\nSilahkan Masuk Ya Kak ${data.pushname}`, MessageType.text, {quoted: data.message}) 
    } catch(e) {
      data.reply('' + e)
    }
  })
              Client.cmd.on('sourcecode', async(data) => {
              data.reply(`Bot Ini Menggunakan Kode Sumber\n\nhttps://github.com/Dvnz99/tes`) 
              }) 
