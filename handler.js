    ...
    try {
        ...
        Client.cmd.on('ytmp4', async (data) => {
            try {
                ...
                res = await axios.get(`https://api.xteam.xyz/dl/ytmp4?url=${data.body}&APIKEY=${configs.xkey}`)
                if(res.data.status == false) data.reply(res.data.message)
                ytm = res.data
                teks = `*Data berhasil didapatkan!*\n\n*Judul* : ${ytm.judul}\n*Ukuran* : ${ytm.size}\n*Ext* : mp4\n\n_Silahkan tunggu file media sedang dikirim mungkin butuh beberapa menit_`
                if(Number(ytm.size.split(' MB')[0]) >= 50.00) return Client.sendFileFromUrl(data.from, `${ytm.thumbnail}`, 'thumb.jpg', `*Data Berhasil Didapatkan!*\n\n*Judul* : ${ytm.judul}\n*Ukuran* : ${ytm.size}\n*Ext* : mp4\n*Link* : ${ytm.url}\n\n_Jika durasi melebihi batas akan dikirim berbentuk link._`, data.message)
                Client.sendFileFromUrl(data.from, `${ytm.thumbnail}`, 'thumb.jpg', teks, data.message)
                Client.sendFileFromUrl(data.from, `${ytm.url}`, `${ytm.judul} - Download.mp4`, `Video telah terkirim @${data.sender.split('@')[0]}`, data.message)
            } catch {
        ...
        Client.cmd.on('ytmp3', async (data) => {
            try {
                ...
                data.reply(mess.wait)
                res = await axios.get(`https://api.xteam.xyz/dl/ytmp3?url=${data.body}&APIKEY=${configs.xkey}`)
                if(res.data.status == false) data.reply(res.data.message)
                ytm = res.data
                teks = `*Data berhasil didapatkan!*\n\n*Judul* : ${ytm.judul}\n*Ukuran* : ${ytm.size}\nExt : mp3\n\n_Silahkan tunggu file media sedang dikirim mungkin butuh beberapa menit_`
                if(Number(ytm.size.split(' MB')[0]) >= 50.00) return Client.sendFileFromUrl(data.from, `${ytm.thumbnail}`, 'thumb.jpg', `*Data Berhasil Didapatkan!*\n\n*Judul* : ${ytm.judul}\n*Ukuran* : ${ytm.size}\n*Ext* : mp3\n*Link* : ${ytm.url}\n\n_Jika durasi melebihi batas akan dikirim berbentuk link._`, data.message)
                Client.sendFileFromUrl(data.from, `${ytm.thumbnail}`, 'thumb.jpg', teks, data.message)
                Client.sendFileFromUrl(data.from, `${ytm.url}`, `${ytm.judul} - Download.mp3`, ``, data.message)
            } catch {
                data.reply('Ups maaf server sedang error')
            }
        }) 
